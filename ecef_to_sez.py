...
# 
# Parameters:
#  o_x_km: description of argument 1
#  o_y_km: description of argument 2
#  o_z_km: 
#  x_km: 
#  y_km:
#  z_km:
...
y_km = float('nan') # y coord of ECEF position
z_km = float('nan') # z coord of ECEF position
...
# Station vector in ECEF 
r_o_ECEF = [ [o_x_km],
             [o_y_km],
             [o_z_km]
]

# Object Vector in ECEF
r_ECEF = [ [x_km],
           [y_km],
           [z_km]
]

r_Station_to_Object_ECEF = np.subtract(r_ECEF , r_o_ECEF) # From Lecture 6 notes

r_x_km = r_Station_to_Object_ECEF[0][0]
r_y_km = r_Station_to_Object_ECEF[1][0] 
r_z_km = r_Station_to_Object_ECEF[2][0]
...
# Rotation matrices
Rzinverse = np.array([[math.sin(lat_rad),  0, -math.cos(lat_rad)],
                  [0,                  1,  0],
                  [math.cos(lat_rad),  0,  math.sin(lat_rad)]])

Ryinverse = np.array([[math.cos(lon_rad), math.sin(lon_rad), 0],
                  [-math.sin(lon_rad), math.cos(lon_rad), 0],
                  [0,                  0,                1]])

# Matrix Multiplication:

r_SEZ = Rzinverse @ Ryinverse @ r_Station_to_Object_ECEF
